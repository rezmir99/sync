...
# chmod and link
## sh
find "$PERM_GIT_PATH/Program/Shell" \( -name '*.lib.sh' \) -prune -o -type f -name '*.sh' -print | while IFS= read -r line; do
    chmod 755 "$line"
    line_name=${line##*/}
...
## lib.sh
find "$PERM_GIT_PATH/Program/Shell" -type f -name '*.lib.sh' -print | while IFS= read -r line; do
    line_name=${line##*/}
    ln -sf "$line" "/usr/local/lib/$line_name"
done

## phyton
find "$HOME/Program/Python" \( -name '*.lib.py' \) -prune -o -type f -name '*.py' -print | while IFS= read -r line; do
    chmod 755 "$line"
    line_name=${line##*/}
